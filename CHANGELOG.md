...
- Subscription summary now prints `Last event: <span> since dial` instead of `Last event latency: <ms>`, with the span rendered in ms/s/m/h as appropriate. The value was always time from dial to the most recent message (stream age), not a latency; the label now says so. JSON `last_event_ms` is unchanged. The summary also gains a `Total bytes:` line (sum over subscriptions).
